...
echo ""
echo "=== Non-exercise slide counts per deck ==="
total_slides=0
for f in slides/0*.qmd; do
  h1=$(grep -cE "^# " "$f")
  ...
  h2=$(grep -cE "^## " "$f")
  content_h2=$((h2 - yt))
  # Non-exercise slides = all H1 + content H2 + 1 auto title slide (from YAML).
  non_ex=$((h1 + content_h2 + 1))
  echo "$f: H1=$h1, content H2=$content_h2, Your turn=$yt, non-exercise total=$non_ex"
...
```
| Deck | Countdowns (min) | Non-exercise slides | Low estimate (1 min/slide) | High estimate (2 min/slide) |
|---|---|---|---|---|
| 01 ... | 40 | 22 | 62 | 84 |
...
| Total | 220 | 107 | 327 | 434 |
```
