...
chat_with_completion_log_probs <- function(
  ...
  # ellmer's chat_openai() uses OpenAI's Responses API, so the per-token
  # logprobs live on the output_text content part of the last raw response
  # (not the old chat-completions `choices[[1]]$logprobs$content`).
  body <- httr2::resp_body_json(httr2::last_response())
  message <- Filter(\(o) o$type == "message", body$output)[[1]]
  text_part <- Filter(\(p) p$type == "output_text", message$content)[[1]]
...
ui <- page_navbar(
  ...
  sidebar = sidebar(
    ...
    textAreaInput(
      ...
      value = "Write a funny limerick about a cougar.",
      rows = 4,
      autoresize = TRUE,
    ),
    input_task_button(
